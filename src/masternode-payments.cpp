...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake)
{
    ...
    if (hasPayment) {
        ...
        if (fProofOfStake) {
            ...
            txNew.vout[i].nValue = masternodePayment  + (nFees/2);

            //subtract mn payment from the stake reward
            txNew.vout[i - 1].nValue -= masternodePayment  + (nFees/2);
        } else {
            txNew.vout.resize(2);
            txNew.vout[1].scriptPubKey = payee;
            txNew.vout[1].nValue = masternodePayment  + (nFees/2);
            txNew.vout[0].nValue = blockValue - masternodePayment  + (nFees/2);
        }
